const BIAS: i32 = 127;
const RADIX: f32 = 2.0;

fn main() {
    let f: f32 = 42.42;

    let (sign, exp, frac) = to_parts(f);
    println!("Sign:{}, Exp: {}, Frac:{}", sign, exp, frac);

    let (sign_, exp_, mant) = decode(sign, exp, frac);
    println!("Sign:{}, Exp: {}, Frac:{}", sign_, exp_, frac);

    let f_ = from_parts(sign_, exp_, mant);

    println!("{} -> {}", f, f_);
    println!("  field  | as bits | as real number");
    println!("  sign   | {:^7b} | {:^14}", sign, sign_);
    println!("exponent | {:08b} | {:^13}", exp, exp_);
    println!("mantissa | {:023b} | {}", frac, mant);
}

fn from_parts(sign: f32, exponent: f32, mantissa: f32) -> f32 {
    sign * exponent * mantissa
}

fn to_parts(f: f32) -> (u32, u32, u32) {
    let bits = f.to_bits();

    let sign = (bits >> 31) & 1;

    let exponent = (bits >> 23) & 0xff;
    let fraction = bits & 0x7fffff;

    (sign, exponent, fraction)
}

fn decode(sign: u32, exponent: u32, fraction: u32) -> (f32, f32, f32) {
    let sign_1 = (-1.0_f32).powf(sign as f32);

    let exponent = (exponent as i32) - BIAS;
    let exponent = RADIX.powf(exponent as f32);

    let mut mantissa = 1.0_f32;
    for i in 0..23 {
        let mark = 1 << i;
        let one_at_bit_i = fraction & mark;
        if one_at_bit_i != 0 {
            let i_ = i as f32;
            let weight = (2_f32).powf(i_ - 23.0);
            mantissa += weight;
        }
    }

    (sign_1, exponent, mantissa)
}
